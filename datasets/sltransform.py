...
class AdjustContrast:
    ...
    def __call__(self, img, target):
        ...
        """
        _contrast_factor = ((random.random() + 1.0) /
                            2.0) * self.contrast_factor
...
class AdjustBrightness:
    ...
    def __call__(self, img, target):
        ...
        img (PIL Image or Tensor): Image to be adjusted.
        """
        _brightness_factor = ((random.random() + 1.0) /
                              2.0) * self.brightness_factor
...
class Rotate:
    ...
    def __call__(self, img, target):
        ...
        target['boxes'] = box_xyxy_to_cxcywh(
            boxes_new).to(boxes_xyxy.dtype) / (whwh + 1e-3)
        return img, target
...
class RandomCrop:
    ...
    def __call__(self, img, target):
        ...
        try:
            ...
            img, new_boxes, new_labels, _ = random_crop(
                img, boxes_xyxy, labels)
    ...
    def __call__(self, img, target, p=1.0):
        ...
        if self.transform and random.random() < p:
            new_res = self.transform(
                image=img_np, bboxes=boxes_raw, class_labels=labels_raw)  # transformed
            boxes_new = torch.Tensor(new_res['bboxes']).to(
                boxes_raw.dtype).reshape_as(boxes_raw)
            img_np = new_res['image']
            labels_new = torch.Tensor(
                new_res['class_labels']).to(labels_raw.dtype)
