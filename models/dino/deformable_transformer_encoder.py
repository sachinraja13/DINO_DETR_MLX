...
class TransformerEncoder(nn.Module):
    def __init__(self, 
        ...
        # Prepare layers
        if num_layers > 0:
            self.layers = _get_encoder_clones(encoder_layer, num_layers, layer_share=enc_layer_share)
        else:
            self.layers = []
            del encoder_layer
        ...
        if two_stage_type in ['enceachlayer', 'enclayer1']:
            ...
            if two_stage_type == 'enclayer1':
                ...
                self.enc_norm = [nn.LayerNorm(d_model) for _ in range(num_layers - 1)]
                self.enc_proj = [nn.Linear(d_model, d_model) for _ in range(num_layers - 1)]
            self.class_embed = [nn.Linear(d_model, 91) for _ in range(num_layers - 1)]
    ...
    def __call__(self, 
        ...
        if ref_token_index is not None:
            out_i = output[mx.arange(batch_size)[:, None, None], ref_token_index[:, :, None], mx.arange(self.d_model)[None, None, :]]
