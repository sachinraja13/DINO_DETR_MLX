...
from .stable_dino_criterion import StableDINOCriterion
import copy
...
def build_stable_dino_loss_criterion(args, matcher):
    # prepare weight dict
    num_classes = args.num_classes
    weight_dict = {'loss_class': args.cls_loss_coef,
                   'loss_bbox': args.bbox_loss_coef}
    ...
    criterion = criterion = StableDINOCriterion(
        ...
        eos_coef=args.eos_coef, loss_class_type=args.cost_class_type,
        focal_alpha=args.focal_alpha, focal_gamma=args.focal_gamma,
        ta_alpha=args.ta_alpha, ta_beta=args.ta_beta,
        two_stage_binary_cls=args.two_stage_binary_cls,
        use_dn=args.use_dn, use_ce_loss_type=args.use_ce_loss_type, stg1_assigner=args.stg1_assigner,
        enc_kd_loss_weight=args.enc_kd_loss_weight, enc_kd_loss_gamma=args.enc_kd_loss_gamma,
        target_post_process=args.target_post_process,
        pad_labels_to_n_max_ground_truths=args.pad_labels_to_n_max_ground_truths,
        n_max_ground_truths=args.n_max_ground_truths,
    )
    return criterion
...
def build_loss_criterion(args, matcher):
    if args.loss_criterion == 'BaseCriterion':
        return build_base_criterion(args, matcher)
    elif args.loss_criterion == 'TwoStageCriterion':
        return build_two_stage_criterion(args, matcher)
    elif args.loss_criterion == 'DINOCriterion':
        return build_dino_loss_criterion(args, matcher)
    elif args.loss_criterion == 'StableDINOCriterion':
        return build_stable_dino_loss_criterion(args, matcher)
    else:
        raise ValueError(f"Unknown criterion: {args.loss_criterion}")
